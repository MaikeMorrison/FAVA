...
You can read your data into R using a function such as `read.csv`. You may also want to confirm that the right $K$ rows of your matrix each sum to 1. Users of _phyloseq_ can use the _FAVA_ function `relab_phyloseq` to generate a relative abundance matrix from a _phyloseq_ object. `relab_phyloseq` combines an OTU table with metadata from the sample data table, creating a data frame with rows corresponding to samples, metadata columns on the left, and OTU relative abundance columns on the right. Sample code for these tasks is provided below. 
...
my_data = relab_phyloseq(phyloseq_object) 
```
