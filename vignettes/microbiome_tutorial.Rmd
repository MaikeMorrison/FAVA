...
These three transformations are plotted below for a maximum pairwise distance of approximately 3.7 $(\max{(D_{i,j})}\approx 3.7)$, the maximum distance of our example data. 
...
We see from the above density plot and the summary at the end of the preceding section that the phylogenetic distance between most species is between about 2 and 3. In this region, the exponential transformation yields much lower similarities than ether the difference or the inverse similarities. 
...
* The **inverse** transformation is similar to the difference distribution for this range of phylogenetic distances (0-3.7), but will be more similar to the exponential distribution for data with a higher maximum phylogenetic distance ($\max{(D_{i,j})}$).  

    * Its distribution of pairwise similarities (b) has a narrow peak like the exponential transformation, but a center around 0.3 like the difference transformation. This shift is also reflected by its large mean similarity value of **0.32**, which is similar to that of the difference transformation (c). 
...
We first compute FAVA across all samples for each subject using each of the three different similarity matrices (difference, exponential, and inverse). 

```{r, echo = FALSE}
rbind(fava(xue_microbiome_sample, group = "subject", time = "timepoint", S = difference_similarity),
      fava(xue_microbiome_sample, group = "subject", time = "timepoint", S = exponential_similarity),
      fava(xue_microbiome_sample, group = "subject", time = "timepoint", S = inverse_similarity)) %>%
  data.frame() %>%
  mutate(Transformation = c("Difference", "Exponential", "Inverse")) %>%
  pivot_longer(-4, names_to = "Subject", values_to = "FAVA") %>%
...
We find that the exponential and inverse transformations yield very similar results. The difference transformation yields FAVA values much lower than the other transformations for subject XDA and XMA, but not for subject XBA. 
...
We find that the results across transformations are qualitatively the same, despite small differences: XBA has the highest peak variability and does not stabilize, while XDA and XMA both stabilize at FAVA values similar to their initial values. 

In order to more easily compare the three transformations, we plot only the study day corresponding to the center of the sliding window 
...
ggplot() +
  geom_polygon(aes(x = x, y= y), 
               ...
               fill = "grey", alpha = 0.7) + 
  geom_point(aes(x = w1 + (w6 - w1)/2, y = FAVA, color = Transformation), fava_window_transformation,
             alpha = 0.5, size = 2) + 
  geom_line(aes(x = w1 + (w6 - w1)/2, y = FAVA, color = Transformation), fava_window_transformation,
            size = 1, alpha = 0.5)  + 
  facet_wrap(~ group) +
  theme_bw()+ 
  scale_color_manual(values = pal) + 
  xlab("Study day of sliding window center") +
   ylab("Weighted FAVA")
```
