...
set.seed(1)
ransamp = sample(1:77,77)
Q_abx = dplyr::mutate(Q, .before = timepoint,
                      abx = ifelse(timepoint<0, "Before",
                                   ifelse(timepoint>6, "After",
                                          "During")))
test_that("window_fava works - unnormalized, MULTIPLE groups, time-weighted and out of order", {
  expect_no_error(window_fava(relab_matrix = Q_abx[ransamp,], group = c("subject", "abx"), time = "timepoint",
                                                                    K = 524, window_size = 8, window_step = 2))
                  expect_true(is.list(window_fava(relab_matrix = Q_abx[ransamp,], group = c("subject", "abx"), time = "timepoint",
                                                  K = 524, window_size = 8, window_step = 2)))
})
